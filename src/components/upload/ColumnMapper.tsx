                        ...
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all ${
                                isActive
                                    ...
                                    : 'bg-th-bg-subtle/50 text-th-text-secondary border border-th-border hover:border-th-border-strong'
                            }`}
                        >
                            <config.icon className="w-3 h-3" />
                            <span className="capitalize">{role}</span>
                            <span className={`px-1.5 py-0.5 rounded ${isActive ? 'bg-white/10' : 'bg-th-bg-elevated'}`}>
                                {count}
                            </span>
                        </button>
                    );
...
function StatBadge({
    ...
    return (
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-th-bg-subtle/50 rounded-lg">
        ...
        >
            <div className="p-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-th-text-muted truncate mt-0.5" title={column.reasoning}>
                        {column.reasoning || 'No description'}
                    </p>
                </div>

                {/* Arrow */}
                <ArrowRight className="w-4 h-4 text-th-text-disabled flex-shrink-0" />
                ...
                <div className="flex-shrink-0">
                    {isEditing ? (
                        ...
                        <button
                            onClick={onEdit}
                            className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-sm font-medium transition-all hover:bg-th-bg-subtle ${roleConfig.bg} ${roleConfig.border} ${roleConfig.color}`}
                        >
                ...
                <div className="flex-shrink-0 w-16">
                    <div className="flex items-center gap-1.5">
                        <div className="flex-1 h-1.5 bg-th-bg-subtle rounded-full overflow-hidden">
                            <motion.div
                                initial={{ width: 0 }}
                ...
                <div className="flex-shrink-0 flex items-center gap-1">
                    {!isIgnored && (
                        <button
                            onClick={onIgnore}
                            className="p-1.5 text-th-text-muted hover:text-th-text-secondary hover:bg-th-bg-subtle rounded-lg transition-colors"
...
function MappingDropdown({
    ...
    return (
        <motion.div
            ...
            className="absolute right-0 top-full mt-1 z-50 w-64 bg-th-bg-subtle border border-th-border rounded-xl shadow-2xl overflow-hidden"
        >
            {/* Search */}
            ...
            <div className="max-h-64 overflow-y-auto p-1">
                {Object.entries(groupedOptions).map(([category, options]) => (
                    <div key={category}>
                        <div className="px-2 py-1 text-[10px] font-semibold text-th-text-muted uppercase tracking-wider">
                            {category}
                        </div>
                        {options.map((opt) => {
                ...
                {filteredOptions.length === 0 && (
                    <div className="py-4 text-center text-th-text-muted text-sm">No matches found</div>
                )}
            </div>
        </motion.div>
    );
